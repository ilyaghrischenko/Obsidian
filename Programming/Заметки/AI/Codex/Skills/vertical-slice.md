...
- **Dependency Injection & Auto-Registration:** You MUST explicitly use the `[FromServices]` attribute for ALL injected dependencies inside the Minimal API `Handle` method parameters (e.g., `[FromServices] Validator validator`, `[FromServices] AppDbContext db`). Do NOT rely on implicit DI resolution. Do NOT manually register types in `Program.cs`. Ensure the `Validator` and `Handler` (if used) explicitly implement the custom `IScopedType` interface to guarantee automatic DI registration.
